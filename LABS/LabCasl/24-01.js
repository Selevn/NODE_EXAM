...
app.post('/login', async (req, res) => {
   ...
   if (candidate) {
       const {id, username, role} = candidate
       const accessToken = jwt.sign({id, username, role}, accessKey, {expiresIn: 10 * 60})
       const refreshToken = jwt.sign({id, username, role}, refreshKey, {expiresIn: 24 * 60 * 60})
    ...
    if (req.cookies.refreshToken) {
        jwt.verify(req.cookies.refreshToken, refreshKey,async (err, payload) => {
            if (err) console.log(err.message)
                ...
                oldRefreshKeyCount++
                const candidate = await UsersCASL.findOne({ where: { id: payload.id } })
                const newAccessToken = jwt.sign({id: candidate.id, username: candidate.username, role: candidate.role}, accessKey, {expiresIn: 3 * 60 * 60})
...
app.get('/api/ability', (req, res) => res.status(200).send(req.ability.rules))
...
app.get('/api/repos/:id', async (req,res) => {
    try {
        ...
        const repo = await Repos.findOne({ where: { id: req.params.id } })
...
app.delete('/api/repos/:id', async (req, res) => {
    try {
        req.ability.throwUnlessCan('manage', 'all')
        await Repos.destroy({ where: { id: req.params.id } })
...
app.put('/api/repos/:id/commits/:commitId', async (req, res) => {
    try {
        req.ability.throwUnlessCan('update', await Repos.findByPk(req.params.id))
        await Commits.update({ message: req.body.message },{
            where: { id: req.params.commitId },
            include: [{
                model: Repos,
                required: true,
...
app.use((req, res, next) => res.status(404).send('Incorrect Method or URL'))

sequelize.sync().then(() => { app.listen(3000) }).catch(error => console.log(error)).then(()=>{
    console.log('started')
})